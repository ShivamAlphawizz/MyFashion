...
import 'package:eshop_multivendor/Screen/DeliverToAddress.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    ...
    cartList[index].perItemTotal =
        (price * double.parse(cartList[index].qty!)).toString();
    if (_editControllers.length < index + 1) {
  ...
  Future<void> addToCartCheckout(String editPrice,
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("checking response here final ${parameter}");
        Response response =
            await post(manageCartApi, body: parameter, headers: headers)
                .timeout(Duration(seconds: timeOut));
    ...
    if (_isNetworkAvail) {
      ...
      try {
        var parameter = {
          ...
          'to_address': LastResult.toString(),
        ...
        print("oooooooooooooooooooooo ${parameter}");
